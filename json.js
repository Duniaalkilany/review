...
===data format get usually from API's===>data format (array contain objects)===>but type of it string(text format ) '[{},{}]'
//why jsom ??
...
weather.map((ele, idx, arr) => {
  console.log(
    ...
    ele["city_name"],
    ele.lon,
    ele.data[0].weather["description"]
  );
});
...
//data shared between server , client in text format (json)
//
//if client recieve json from server===>parse it (convert it js obj to deal with it (access it))
...
document.getElementById("name").innerHTML = jsoO.slice(
  jsoO.indexOf("d"),
  jsoO.indexOf(",") - 1
); //will print json obj (string){"name":"hussam","age":23}

//if i want to make json object
/*
extension .json
no need to add '' in json files because it know that is json 
[{},{},{}]
but not that ket should wraapeed in double quotes /if value is string also wrrapped it with double quotes
*/
//===========================using json with XMLHttpRequest=========================//
const request = new XMLHttpRequest();
let div = document.getElementById("myJson");
...
request.addEventListener("readystatechange", function () {
  if (request.readyState === 4 && request.status === 200) {
    ...
    jsData.map((ele, idx) => {
      ...
      const text = document.createTextNode(`${idx + 1}-${ele.userName}`);
      element.appendChild(text);
      div.appendChild(element);
    });
  }
});
//here i hit json file using XMLHttpRequest
